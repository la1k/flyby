...
*****************************************************************************
*                                                                           *
*           This code was last modified by KD2BD on 28-Feb-2003.            *
*                                                                           *
*****************************************************************************
*                                                                           *
...
char *vis2color(visibility)
{
	...
	{
		case 'D':
		strcpy(string,"color=white # In sunlight");
		break;

		case 'N':
		strcpy(string,"color=blue # In darkness");
		break;

		case 'V':
		strcpy(string,"color=yellow # Optically visible");
		break;
	}
...
char argc, *argv[];
{
	...
	char buf[128], command[255], satlist[625], satname[26],
	     ...
	     greatarcfile[50], sat2track[30], hostname[50], color[35],
	     xtra[50], xplanet=0, xearth=0, visibility, circledrawn=0,
	     zoom=0;
	...
	while (error==0)
	{
		...
		for (z=0; z<satnum; z++)
		{
			...
			if (strcmp(satnamelist[z],sat2track)==0 && range>0.0)
			{
				...
				if (zoom)
					radius=(int)(100.0*(LOCAL_R0/footprint));
				else
					radius=50;
		...
		if (error==0)
		{
			...
			{
				if (circledrawn)
					sprintf(command,"xplanet -proj orth -markerfile %s -greatarcfile %s -longitude %f -latitude %f -radius %d %s",markerfile,greatarcfile,mapcenterlong,mapcenterlat,radius,xtra);
				else
					sprintf(command,"xplanet -proj orth -markerfile %s -latitude %f -longitude %f %s",markerfile,mapcenterlat,mapcenterlong,xtra);
			}
