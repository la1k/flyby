...
char argc, *argv[];
{
	...
	while (error==0)
	{
		...
		if (error==0)
		{
			...
			{
				if (circledrawn)
					sprintf(command,"xplanet -config %s -proj orth -longitude %f -latitude %f -radius %d %s",configfile,mapcenterlong,mapcenterlat,radius,xtra);
				else
					sprintf(command,"xplanet -config %s -proj orth -latitude %f -longitude %f %s",configfile,mapcenterlat,mapcenterlong,xtra);
			}
