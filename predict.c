	...
	do
	{
		...
		if (sat_ele>=0.0)
		{
			...
			if (speak=='T' && soundcard)
			{
				if (eclipse_alarm==0 && fabs(eclipse_depth)<0.015) /* ~1 deg */
				{
					...
					{
						sprintf(command,"%s/vocalizer eclipse &",predictpath);
						system(command);
						eclipse_alarm=1;
					...
					{
						sprintf(command,"%s/vocalizer sunlight &",predictpath);
						system(command);
						eclipse_alarm=1;
						oldtime-=0.000015*sqrt(sat_alt);
					}
				...
				if ((CurrentDaynum()-oldtime)>(0.00003*sqrt(sat_alt)))
				{
					...
					sprintf(command,"%s/vocalizer %.0f %.0f %c %c &",predictpath,sat_azi,sat_ele,approaching,visibility);
					system(command);
  					oldtime=CurrentDaynum();
		...
		else if (sat_ele<0.0 && geostationary==0 && decayed==0 && aoshappens==1 && daynum>aoslos)
		{
			...
			if (oldtime!=0.0 && speak=='T' && soundcard)
			{
				...
				sprintf(command,"%s/vocalizer los &",predictpath);
				system(command);
			}
		}
...
time_to_die( int why )
{
  /* syslog ( LOG_INFO, "Time_to_Die called (%d)\n", why ); */
   exit ( -1 );
}

void MultiTrackDaemon()
{
	/* This function tracks all satellites in the program's
	   database simultaneously until 'Q' or ESC is pressed.
	   Satellites in range are HIGHLIGHTED.  Coordinates
	...
	/* openlog ( "predict", LOG_PID, LOG_USER );
	   syslog ( LOG_INFO, "Starting the satellite Prediction server." ); */
	...
	pthread_create(&thread,NULL,(void *)socket_server,(void *)"predict");
	...
	do
	{
		...
		sleep(1);  /* Increase if CPU load is too high */
...
char argc, *argv[];
{
	...
	if (interactive)
	{
		...
		if (getenv("TERM") && strncmp(getenv("TERM"),"xterm",5)==0)
		  xterm=1;
		else
		  xterm=0;

		if (!getenv("TERM") || strncmp(getenv("TERM"),"none",4)==0) {
		  /* NO-OP */;
		} else {
		initscr();
		start_color();
